...
for (FeatNum in seq(from=100, to=10000, by=200)){
	print(FeatNum)
	CLASSNumber=19
	start_time <- Sys.time()
	...
	save(rf, file=paste("RF_model.pca.test_",FeatNum,"_Genes.PCA.Robj",sep = ""))
	...
	#Random Gene Selection
        rand.genes <- as.character(sample(rownames(pcatrain$rotation),  length(pca.genes) ))
        ...
        save(rand.AccuracyTable, file="rand.AccuracyTable_FeatureTest.Robj")
        save(rand.errorsTable, file="rand.ErrorsTable_FeatureTest.Robj")
...
pacc <- ggplot(AccuracyTable)+geom_line(aes(x=FeatureNumber, y=Accuracy),color="blue")+ylim(c(0,100))+geom_line(aes(x=FeatureNumber, y=rand.Accuracy),color="red")
...
ggsave(plot=pacc,width = 12,height = 8, dpi=200, filename="accuracyPlot_FeatureTest.pdf", useDingbats=FALSE )
ggsave(plot=perr, width = 20,height = 8, dpi=200, filename="ClassErrorPlot_FeatureTest.pdf", useDingbats=FALSE )
